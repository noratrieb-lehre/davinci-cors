...
export default class UserService {
    ...
    public constructor() {
        ...
        if (refreshToken) {
            ...
            axios.get(`${process.env.REACT_APP_BACKEND_URL}/token` || 'http://localhost:8080/api/token', {
                headers: {
    ...
    private async updateAuthToken(): Promise<AxiosResponse> {
        const response = await axios.get(`${process.env.REACT_APP_BACKENDURL}/token` || `${window.location.protocol}//${window.location.host}:8080/api/token`, {
            headers: {
