...
async fn show_all_events(ctx: &Context, interaction: &Interaction) -> BotResult<()> {
    let events = get_events(ctx, None, None).await?;
...
async fn show_next_events(ctx: &Context, interaction: &Interaction) -> BotResult<()> {
    ...
    let events = get_events(ctx, None, current_time).await?;

    let events = events
        .into_iter()
        .filter(|event| event.end > current_time)
        .collect::<Vec<_>>();

    send_events(ctx, interaction, events.as_slice()).await
}

async fn show_filter_events(ctx: &Context, interaction: &Interaction) -> BotResult<()> {
    let events = get_events(ctx).await?;
}
